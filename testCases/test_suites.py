...
@pytest.mark.parametrize("product", products_scenario)
def test_pick_products(product, setup):
    driver, wait = setup
    ...
    # get login page
    lp.login_link()
    lp.set_input_text(username_el, "John")
    lp.set_input_text(password_el, "supertest")
    lp.login()
